...
export default function SubcontractorDetailScreen() {
  ...
  async function removeAssignmentRow(a: SubcontractorAssignmentWithProject) {
    const ok = await confirm('Retirer ce sous-traitant de ce chantier ?', a.projects?.name ?? '');
    if (!ok) return;
  ...
  async function handleDeleteCompany() {
    if (!subcontractor) return;
    const ok = await confirm('Supprimer cette entreprise ?', `${subcontractor.company_name} et toutes ses affectations seront définitivement supprimées.`);
    if (!ok) return;
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: spacing.xxl * 2 }}>
        <View style={styles.container}>
          <PageHeader
            ...
            right={<RowActionMenu actions={[{ key: 'delete', icon: 'trash-2', label: 'Supprimer', danger: true, onPress: handleDeleteCompany }]} />}
          />
          ...
          <Card style={{ gap: spacing.sm }}>
            ...
            <Button title="Enregistrer" icon="check" onPress={saveIdentity} loading={savingIdentity} />
          </Card>

          <Text style={styles.sectionTitle}>Attestation d'assurance</Text>
          <Card style={{ gap: spacing.sm }}>
            {subcontractor.insurance_doc_path ? (
              <View style={styles.insuranceRow}>
                ...
                <Text style={[styles.insuranceText, insuranceExpired && { color: colors.danger }]}>
                  {insuranceExpired ? 'Attestation expirée' : 'Attestation à jour'}
                </Text>
              ...
              <Text style={styles.meta}>Aucune attestation d'assurance téléversée.</Text>
            )}
            <DateField label="Date d'expiration" value={expiry} onChange={setExpiry} />
            <Button
              title={subcontractor.insurance_doc_path ? 'Remplacer le document' : 'Téléverser le document'}
              variant="secondary"
              icon="upload"
            ...
            {uploadSuccess ? (
              <View style={styles.insuranceRow}>
                ...
                <Text style={styles.insuranceText}>Attestation téléversée avec succès.</Text>
              </View>
            ) : null}
          </Card>
          ...
          <View style={styles.sectionHeaderRow}>
            ...
            <Pressable onPress={openAssignPicker}>
              <Text style={styles.link}>+ Assigner à un chantier</Text>
            </Pressable>
          </View>
          {assignments.length === 0 ? (
            <EmptyState title="Aucun chantier" subtitle="Cette entreprise n'est encore assignée à aucun chantier." />
          ) : (
                ...
                return (
                  <Card key={a.id} style={{ padding: 0, overflow: 'hidden' }}>
                    <Pressable onPress={() => toggleExpand(a)} style={styles.assignmentRow}>
                      <View style={{ flex: 1 }}>
                        <Text style={styles.assignmentProject}>{a.projects?.name ?? 'Chantier'}</Text>
                    ...
                    {expanded ? (
                      <View style={styles.assignmentExpanded}>
                        ...
                        <View style={styles.assignmentActions}>
                          <Button title="Voir le chantier" variant="secondary" onPress={() => router.push(`/(app)/chantiers/${a.project_id}`)} style={{ flex: 1 }} />
                          <Button title="Retirer" variant="danger" onPress={() => removeAssignmentRow(a)} style={{ flex: 1 }} />
                        </View>
                        <Button title="Enregistrer" icon="check" onPress={() => saveAssignmentEdit(a.id)} loading={savingAssignment} />
                      </View>
                    ) : null}
                  </Card>
                );
          ...
          <Text style={styles.sectionTitle}>Factures reçues</Text>
          {invoices.length === 0 ? (
            <Text style={styles.meta}>Aucune facture reçue pour l'instant.</Text>
          ) : (
              ...
              {invoices.map((inv) => (
                <View key={inv.id} style={styles.invoiceRow}>
                  <Pressable style={{ flex: 1 }} onPress={() => openInvoiceFile(inv)}>
                    ...
                    <Text style={styles.meta}>
                      {[inv.project_subcontractors?.projects?.name, inv.amount != null ? `CHF ${inv.amount.toLocaleString('fr-CH', { minimumFractionDigits: 2 })}` : null, displayDate(inv.invoice_date)]
                        .filter(Boolean)
                        .join(' · ')}
                    </Text>
                  </Pressable>
          ...
          {assignments.length > 0 ? (
            <Card style={{ gap: spacing.sm }}>
              {assignments.length > 1 ? (
                <View>
                  <Text style={styles.fieldLabel}>Chantier concerné</Text>
              ...
              <Button title="Ajouter une facture" variant="secondary" icon="upload" onPress={pickInvoice} loading={uploadingInvoice} />
            </Card>
          ) : (
            <Text style={styles.meta}>Assignez d'abord cette entreprise à un chantier pour pouvoir y attacher des factures.</Text>
          )}
        </View>
      </ScrollView>
        ...
        <View style={styles.sheetOverlay}>
          <View style={styles.sheet}>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Assigner à un chantier</Text>
            ...
            <ScrollView style={{ maxHeight: 420 }}>
              ...
              {assignableProjects.length === 0 ? (
                <Text style={styles.meta}>Déjà assignée à tous vos chantiers.</Text>
              ) : null}
            </ScrollView>
            {assigning ? <Text style={styles.meta}>Ajout en cours…</Text> : null}
          </View>
        </View>
      </Modal>
    </Screen>
