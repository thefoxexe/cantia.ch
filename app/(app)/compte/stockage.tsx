        ...
        <Container>
          ...
          <Card style={styles.summaryCard}>
            <View style={styles.summaryHeader}>
              <Text style={styles.summaryTitle}>{planName ? t('stockage.plan', { name: planName }) : t('stockage.usage')}</Text>
            ...
            <Text style={[styles.summaryText, nearLimit && styles.textDanger]}>
              {t('stockage.usedOf', { used: formatBytes(usedBytes), quota: quotaBytes > 0 ? formatBytes(quotaBytes) : '—' })}
            </Text>
          </Card>
          ...
          {breakdown.length === 0 ? (
            ...
            <Card style={styles.breakdownCard}>
              {CATEGORY_ORDER.filter((key) => breakdown.some((b) => b.category === key)).map((key, i, arr) => {
                ...
                const icon = CATEGORY_ICON[key] ?? CATEGORY_ICON.autre;
                const label = t(`stockage.categories.${key}` as any, { defaultValue: t('stockage.categories.autre') });
          ...
          {isAdmin ? (
            <View style={styles.upgradeCard}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.upgradeTitle}>{t('stockage.upgradeTitle')}</Text>
                <Text style={styles.upgradeText}>{t('stockage.upgradeText')}</Text>
              </View>
            </View>
          ) : null}
          {isAdmin ? (
            <Button
              title={t('stockage.seePlans')}
