-- The client-documents history page lists every devis/facture for one
-- client email and lets them download any of them directly — each has its
-- own public_token, so a session check scoped to the token the code was
-- requested against would break that browsing flow. A verification code
-- only ever proves "this browser controls this email address"; which
-- documents that email may see is still independently enforced by each
-- RPC's own client_email match, so the session itself is rescoped to
-- email-only rather than (token, email).

drop function if exists public.has_valid_document_session(uuid, text, text);

create or replace function public.has_valid_document_session(p_email text, p_session text)
returns boolean
language sql
security definer
...
grant execute on function public.has_valid_document_session(text, text) to anon, authenticated;
