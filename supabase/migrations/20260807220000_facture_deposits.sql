-- Deposit invoices: a devis can now spawn more than one facture — one or
-- more "acompte" (deposit) invoices, each for a percentage of the devis
-- total, followed by a final invoice. The final invoice automatically
-- deducts whatever deposits were already billed on the same devis, as a
-- negative line item, so the client only ever owes the remaining balance.
alter table public.factures add column if not exists is_deposit boolean not null default false;

-- Was "one facture per devis" — no longer holds once deposits exist
-- alongside a final invoice for the same devis.
drop index if exists public.factures_devis_id_key;
create index if not exists factures_devis_id_idx on public.factures (devis_id) where devis_id is not null;
