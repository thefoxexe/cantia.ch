...
import { syncBexioArticles, syncBexioContacts, syncBexioInvoicesFromBexio, syncBexioInvoiceStatuses } from './bexio-sync-logic.ts';
...
// connected and auto_sync_enabled, pulling contacts, articles (into the
// Catalogue), invoices created directly in Bexio, and the payment status
// of already-mapped ones. A product created in Bexio between two manual
// "Synchroniser maintenant" clicks used to only reach the Catalogue on the
// next manual sync or reconnect — now every hourly sweep picks it up too.
