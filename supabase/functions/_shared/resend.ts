...
// Shared branded shell (wordmark header, cream background, footer) used by
...
// Cantia product chrome. Colors match lib/theme.ts. The header is a plain
// text wordmark, not an <img> — an external logo fetch is one more thing
// that can fail (host hiccup, Gmail's image proxy caching a miss) and shows
// as a broken-image glyph when it does; text always renders.
