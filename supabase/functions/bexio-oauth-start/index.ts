...
// V1 scope set (cahier des charges section 8/103), plus contact_edit and
// kb_offer_edit added once client-push and devis-push (Cantia -> Bexio
// contact / kb_offer) shipped. article_edit stays out — Cantia never
// writes articles back, only reads them into the Catalogue. A scope should
// never be requested before the feature using it exists; kb_offer_edit
// already includes kb_offer_show per Bexio's own docs, so that separate
// read scope is never requested.
const BEXIO_SCOPES = 'openid company_profile offline_access contact_show contact_edit article_show kb_invoice_edit kb_offer_edit';
